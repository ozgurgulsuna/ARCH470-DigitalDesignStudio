...
for f in range(DURATION):
    ...
    for c in range(C):
        color = np.dstack((curl[:,c,:], np.ones((A,C)), fluid.dye[:,c,:]))
        color = (np.clip(color, 0, 1) * 255).astype('uint8')
        frames[c].append(Image.fromarray(color, mode='HSV').convert('RGB'))
#        print('Saving simulation result.')

for c in range(C):
    frames[c][0].save('example'+str(c)+'.gif', save_all=True, append_images=frames[c][1:], duration=20, loop=0)
